...
export const PROVIDERS = {
  google: {
    ...
    // 폴백 순서: 최신 → 이전. Google이 모델 EOL 걸어도 하나는 살아있도록.
    fallbackModels: ['gemini-2.0-flash', 'gemini-2.0-flash-exp', 'gemini-1.5-flash', 'gemini-1.5-flash-002', 'gemini-1.5-flash-latest'],
    keyPrefix: 'AI',
  ...
  anthropic: {
    ...
    fallbackModels: ['claude-haiku-4-5-20251001', 'claude-3-5-haiku-20241022', 'claude-3-haiku-20240307'],
    keyPrefix: 'sk-ant-',
...
export async function pingProvider({ provider, apiKey, model }) {
  ...
  for (const m of candidates) {
    try {
      ...
      if (/401|403|API key|invalid|unauthorized|permission/i.test(msg)) {
