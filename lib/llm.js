...
export const PROVIDERS = {
  groq: {
    label: 'Groq Llama 3 (무료·빠름·추천)',
    // 8B-instant 기본: 일 14,400회 vs 70B의 1,000회 (14배)
    // 대규모 동시 접속(수련회 수십명)에 안정적
    defaultModel: 'llama-3.1-8b-instant',
    // 429 시 순차 시도: 8B-instant → llama3-8b → gemma2 → 70B
    fallbackModels: [
      'llama-3.1-8b-instant',   // 14,400 RPD
      'llama3-8b-8192',         // 14,400 RPD
      'gemma2-9b-it',           // 14,400 RPD
      'llama3-70b-8192',        // 14,400 RPD
      'llama-3.3-70b-versatile' // 1,000 RPD (최후 수단)
    ],
    keyPrefix: 'gsk_',
...
export async function sendChat({ provider, apiKey, model, system, messages, maxTokens = 300 }) {
  const meta = PROVIDERS[provider]
  if (provider === 'groq') {
    // 대규모 동시 접속 대응: 명시된 모델이 429면 폴백 체인 순차 시도
    const fullMsgs = [{ role: 'system', content: system }, ...messages]
    const tryOrder = model
      ? [model, ...(meta?.fallbackModels || []).filter(m => m !== model)]
      : (meta?.fallbackModels || [meta?.defaultModel])
    ...
    for (const m of tryOrder) {
      try {
        ...
        lastErr = new Error(`[Groq ${r.status} ${m}] ${errText.slice(0, 200)}`)
        // 429(한도)·503(과부하)이면 다음 모델. 그 외 에러는 즉시 종료
        if (r.status !== 429 && r.status !== 503) throw lastErr
      } catch (e) {
        lastErr = e
        if (!/429|503|rate|quota|unavailable/i.test(e.message || '')) throw e
      }
    }
    throw lastErr || new Error('[Groq] all models rate-limited')
  }
  if (provider === 'google') {
    const genai = new GoogleGenerativeAI(apiKey)
